...
`GET /v1/menu/items/:itemId`. No Cart or Order route exists yet.
...
1. [`../../docs/api/commerce-api.md`](../../docs/api/commerce-api.md) — the
   ...
   and §11 for the Menu routes specifically).
2. [`../../docs/architecture/architecture-decisions.md`](../../docs/architecture/architecture-decisions.md)
   ...
   shapes live, and how a domain error becomes an HTTP response.
...
`infrastructure/` touching storage (ADR-0013 §8). Do not add a Cart or Order
route, a real database, or any dependency outside an approved phase's scope
without a new plan — see
